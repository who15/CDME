...
class ista_unet(nn.Module):
    def __init__(self, kernel_size=3, hidden_layer_width_list=[256, 128, 64], n_classes=64, ista_num_steps=5,
        ...
        self.rec_x_unique = nn.Conv2d(n_classes, 4, 3, 1, 1)
        self.rec_y_unique = nn.Conv2d(n_classes, 4, 3, 1, 1)
        ...
        self.resnet_x = nn.ModuleList(
            [ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[0], kernel_size=3, n_resblocks=1), \
             ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[1], kernel_size=3, n_resblocks=3), \
             ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[2], kernel_size=3, n_resblocks=5)])
        self.resnet_y = nn.ModuleList(
            [ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[0], kernel_size=3, n_resblocks=1), \
             ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[1], kernel_size=3, n_resblocks=3), \
             ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[2], kernel_size=3, n_resblocks=5)])
        self.resnet_z = nn.ModuleList(
            [ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[0], kernel_size=3, n_resblocks=1), \
             ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[1], kernel_size=3, n_resblocks=3), \
             ResidualGroup(conv=default_conv, n_feat=self.hidden_layer_width_list[2], kernel_size=3, n_resblocks=5)])
    ...
    def forward(self, x, y, z):
        ...
        for idx in range(1, self.ista_num_steps):
            ...
            for i in range(self.num_layers):
                ...
                x_list[i] = self.resnet_1[i](self.relu(x_list[i]))
                y_list[i] = self.resnet_2[i](self.relu(y_list[i]))
            ...
            for i in range(self.num_layers):
                ...
                z_list[i] = self.resnet_3[i](self.relu(z_list[i]))
        ...
        return x_unique, y_unique,common
...
class Net(nn.Module):
    def __init__(self, criterion, panchannels, mschannels):
        super(Net, self).__init__()
        self.criterion = criterion
        self.panchannels = panchannels
        self.mschannels = mschannels
        self.channel_fea = 64
        self.layer_in_x = nn.Conv2d(self.panchannels, self.channel_fea, 3, 1, 1)
        self.layer_in_y = nn.Conv2d(self.mschannels, self.channel_fea, 3, 1, 1)
        self.layer_in_z = nn.Conv2d(self.panchannels + self.mschannels, self.channel_fea, 3, 1, 1)
        self.channel_fea = 64
        self.predict_ista = ista_unet(kernel_size=3, hidden_layer_width_list=[128, 96, 64], n_classes=self.channel_fea,
                                      ista_num_steps=5)
        self.decoder = decoder(self.mschannels, 32)
        self.fianl = nn.Conv2d(32,mschannels,1)
        self.exp=CombinedExpertSystem(channels=64, num_experts=3, top_k=3)
    ...
    def forward(self, y, x):
        ...
        x_rec, y_rec,common= self.predict_ista(x_in, y_in, z_in)


        out_put1=self.exp(y_rec,x_rec,common)

        out3=self.decoder(out_put1)
